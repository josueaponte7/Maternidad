$(document).ready(function() {
    ...
    var $pefil = $frmperfil.find($('input:text#perfil'));
            ...
            if (validar($pefil) === true) {
                ...
                $.post(urlperfil, $frmperfil.serialize(), function(data) {
                    ...
                    if (cod_msg === 21) {
                        ...
                        codigo = codigo + 1;
                        TPefil.fnAddData([codigo, $pefil.val(), modificar, eliminar]);
                        limpiar();
                    }
                }, 'json');
            }
                    ...
                    if (respuesta === true) {
                        $.post(urlperfil, $frmperfil.serialize(), function(data) {
                            ...
                            if (cod_msg === 22) {
                                ...
                                TPefil.fnUpdate($pefil.val(), parseInt(fila), 1);
                                limpiar();
                            }
                        }, 'json');
                    }
    ...
    $tabla_perfil.on('click', 'tr > td', function() {
        ...
        if (indice < 2) {
            ...
            var cod = $(this).parents('tr').children('td:eq(0)').text();
            var perfil = $(this).parents('tr').children('td:eq(1)').text();
            var $perfil = ' <input type="hidden" id="cod" name="cod" value="' + cod + '" />';
            var $cod = ' <input type="hidden" id="hperfil" name="hperfil" value="' + perfil + '" />';

            $($cod).prependTo($frmperfil);
            $($perfil).prependTo($frmperfil);

            $btnlistar.removeAttr("disabled");
            $pefil.attr('disabled', 'disabled').val(perfil);
    ...
    /*var $btnlimpiar   = $frmperfil.find($('input:button#btnlimpiar'));
     var $btnlistar    = $frmperfil.find($('input:button#btnlistar'));*/
    var $tabla_perfil = $('#tabla_privilegios');


    var TPrivilegios = $('#tabla_privilegios.dataTable').dataTable({
        "iDisplayLength": 5,
        "iDisplayStart": 0,
        "sPaginationType": "full_numbers",
        "aLengthMenu": [5, 10, 20, 30, 40, 50],
        "oLanguage": {"sUrl": "librerias/js/es.txt"},
        "aoColumns": [
            {"sClass": "agregar center", "sWidth": "5%"},
            {"sClass": "agregar center", "sWidth": "10%"},
            {"sClass": "center"},
            {"sClass": "center"},
            {"sClass": "center"},
    ...
    $btnlistar.on('click', function() {
        $nom_perfil.text($('#hperfil').val());
        var cod_perfil = $('#cod').val();
        var $cod = ' <input type="hidden" id="hperfil" name="hperfil" value="' + cod_perfil + '" />';

        $($cod).prependTo($frmfrmprivilegio);
    ...
    $btnaccpriv.click(function() {
        ...
        var $codigo = '<input type="hidden" id="accion"  value="AgregarPrivilegios" name="accion">';
        ...
        $("input:checkbox[name='privilegio[]']:checked").each(function() {
            ...
            nomb_privilegio.push($(this).next('label').text());
        });

        var privil = privilegios.join(",");
        var nom_privil = nomb_privilegio.join(",");

        $($codigo).prependTo($(this));
    ...
    $tabla_perfil.on('click', 'img.modificar', function() {
        ...
        var cod_pri = $padre.children('td:eq(2)').attr('id');
        alert(cod_pri);
        /*var $cod  = ' <input type="hidden" id="cod" name="cod" value="' + cod + '" />';
         var $fila = '<input type="hidden" id="fila"  value="' + fila + '" name="fila">';
...
function limpiar() {
    ...
    /*$('#accion').remove();
     $('form#frmperfil select#modulo').val(0);
     $('form#frmperfil select#sub_modulo').find('option:gt(0)').remove().end();
     $("input:checkbox[name='privilegio[]']").prop('checked', false);*/
}